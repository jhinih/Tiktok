...
func (l *LoginLogic) Register(ctx context.Context, req types.RegisterRequest) (resp types.RegisterResponse, err error) {
	...
	//查询用户
	var user model.User
	user, err = request.NewLoginRequest(global.DB).GetUserByEmail(req.Email)
...
func (l *LoginLogic) TokenTest(ctx context.Context, req types.TokenTestRequest) (resp types.TokenTestResponse, err error) {
	return
}
